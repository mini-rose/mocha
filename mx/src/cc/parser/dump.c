...
static const char *expr_info(expr_t *expr)
{
	...
	case E_MODULE:
		mod = expr->data;
		snprintf(info, 512, "\033[1;98m%s\033[0m src=%s", mod->name,
			 mod->source_name);
		break;
	case E_FUNCTION:
	...
	case E_VARDECL:
		...
		snprintf(info, 512, "\033[33m%s\033[0m %s", tmp,
	...
	case E_ASSIGN:
		...
		snprintf(info, 512, "%c%s%s = (\033[33m%s\033[0m) %s", marker,
...
void expr_print_value_expr(value_expr_t *val, int level)
{
	...
	case VE_LIT:
		...
		printf("literal: \033[33m%s\033[0m %s\n", tmp, lit_str);
		break;
	case VE_CALL:
...
{
	indent(level);
	printf("\033[93m%s\033[0m { ", o->name);
	for (int i = 0; i < o->n_fields; i++) {
		printf("%s: \033[93m%s\033[0m ", o->field_names[i],
...
static void expr_print_mod_expr(mod_expr_t *mod, int level)
{
	...
	if (mod->n_local_decls) {
		indent(level);
		printf("\033[95mLocal declarations (%d):\033[0m\n",
		       mod->n_local_decls);
	}
	...
	if (mod->n_type_decls) {
		indent(level);
		printf("\033[95mType declarations (%d):\033[0m\n",
		       mod->n_type_decls);
	}
	...
	if (mod->std_modules) {
		if (mod->std_modules->n_modules) {
			indent(level);
			printf("\033[95mStandard modules (%d):\033[0m\n",
