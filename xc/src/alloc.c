/* alloc.c - custom allocators for xc
				   ...
				   int i_slab, int i_pos)
{
	...
	printf("alloc: Buffer sanitizer: \033[1;91m%s\033[0m in slab%d[%d,%d] "
	       "allocator\n",
	...
	if (info->file) {
		...
		if (!src) {
			printf("alloc:   \033[90m<could not open source "
			       "file>\033[0m\n");
			goto skip;
		}
...
skip:
	...
	printf("alloc:             v~~~~~~~\n");
	printf("alloc:   "
	       "|\033[31xcxxxxxxx\033[0m|\033[32m--user--\033[0m|\033["
	       "31xcxxxxxxx\033[0m|\n");
