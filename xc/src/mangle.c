/* xc/mangle.c - mangle functions and types
...
char *xc_mangle_type(type_t *ty, char *buf)
{
	...
	if (ty->kind == TY_PLAIN) {
		...
		buf[0] = 'P';
		xc_mangle_type(ty->v_base, &buf[1]);
	} else if (ty->kind == TY_ARRAY) {
		snprintf(buf, MANGLE_BUF_SIZ, "A_");
		xc_mangle_type(ty->v_base, &buf[2]);
	} else if (ty->kind == TY_OBJECT) {
		/* parse this as the fully qualified name of the type */
		snprintf(buf, MANGLE_BUF_SIZ, "%zu%s",
			 strlen(ty->v_object->name), ty->v_object->name);
	} else {
...
char *xc_mangle(const fn_expr_t *func)
{
	...
	for (int i = 0; i < func->n_params; i++)
		xc_mangle_type(func->params[i]->type, &name[strlen(name)]);
